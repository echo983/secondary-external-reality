...
当前版本：`0.3.0`。完整工程快照见 [上下文交接](docs/CONTEXT-HANDOFF-2026-08-23.md)，文档导航见 [docs 索引](docs/INDEX.md)。
...
- 打开/关闭、拿起、放到表面或容器；
- 在床边与门口两个 Place 之间移动（`move`），位置作为可提交、可查询的世界事实持久化；
...
- 自动化人类模拟测试：带种子的结构化语料日常门禁 + LLM 人设自由探索，配合纯不变量检测器减少真人测试轮次（详见 [空间移动设计](docs/MVP-minimal-space-movement-design-v0.3.md) 与 `src/eval/`）。

尚未开放：门外空间、卧室以外的其他 Place、动态实体创建、任意文字书写、可信能力问答和真正开放世界。`看看门外` 应返回未建模范围，而不是编造门外内容——即使玩家已经站在门口。
