...
export class DeterministicPresentationRenderer implements ApprovedPresentationRenderer {
  async render(packet: ApprovedPresentationPacket, languageSample: string): Promise<string> {
    ...
    if (item.kind === "boundary") {
      const zh = { TARGET_NOT_PERCEIVABLE: "你现在无法感知到目标。", CONTAINER_CLOSED: "容器关着，你现在看不到里面。", NO_ACQUIRED_EVIDENCE: "你没有可供查阅的既有证据。", UNSUPPORTED_PROJECTION: "当前世界还不能回答这个问题。", RESOLUTION_DEFERRED: "这个事实目前尚未固定。", AMBIGUOUS_TARGET: "你指的目标不够明确。", RECOLLECTION_FADED: "你努力回想，但已经记不清了。", OUT_OF_OBSERVATION_BANDWIDTH: "你离得太远，看不清上面写的字。", TESTIMONY_UNAVAILABLE: "室友似乎不知道这件事。" } as const;
      const en = { TARGET_NOT_PERCEIVABLE: "You cannot currently perceive the target.", CONTAINER_CLOSED: "The container is closed, so you cannot see inside.", NO_ACQUIRED_EVIDENCE: "You have no acquired evidence to consult.", UNSUPPORTED_PROJECTION: "The current world cannot answer that yet.", RESOLUTION_DEFERRED: "That fact has not yet been fixed.", AMBIGUOUS_TARGET: "The target is ambiguous.", RECOLLECTION_FADED: "You try to recall, but it's faded from memory.", OUT_OF_OBSERVATION_BANDWIDTH: "You are too far away to make out what is written on it.", TESTIMONY_UNAVAILABLE: "The roommate doesn't seem to know anything about that." } as const;
