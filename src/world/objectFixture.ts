...
export function createObjectWorldFixture(): ObjectWorldFixture {
  const entities: Array<{ id: string; type: string; names: string[]; attributes?: Record<string, string> }> = [
    ...
    // notable_feature is deliberately NOT seeded on either place below — it
    // stays Free until the first time it is operationally addressed (see
    // resolvePlaceNotableFeature in objectTurn.ts), per
    // docs/MVP-hallway-placegraph-design-v0.4.md and -living-room-design-v0.5.md.
    { id: "hallway-1", type: "place", names: ["走廊", "门外", "hallway", "outside"], attributes: { zh_name: "走廊", en_name: "hallway" } },
    { id: "living-room-1", type: "place", names: ["客厅", "living room", "living_room"], attributes: { zh_name: "客厅", en_name: "living room" } },
  ];
