...
test("writes, restarts, finds, and reads the exact inscription", async () => {
  ...
  try {
    ...
    // Deliberately unrelated to the note, but must not move self away from
    // bedside — reading fine print later in this test now requires being at
    // bedside (docs/MVP-observation-bandwidth-design-v0.8.md).
    const unrelated = await firstProcess.submit("打开抽屉");
...
test("reads a legacy paper commit that predates world-basis metadata", async () => {
  ...
  try {
    ...
    assert.equal(read.commitPackage.worldBasis?.fixtureVersion, "0.6.0");
  } finally {
    store.close();
    await rm(directory, { recursive: true, force: true });
  }
